...
public:
	DrawableString (float layerDepth) : DrawableBase(layerDepth) { sfString = new sf::String(); }
	DrawableString (std::string text, std::string fontFile, float size, float layerDepth);
	DrawableString (std::string text, float size, float layerDepth);
