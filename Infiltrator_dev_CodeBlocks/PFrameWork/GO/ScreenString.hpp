...
public:
	ScreenString (std::string text, std::string fontFile, float size, AlignH alignH = LEFT, AlignV alignV = TOP, int layerDepth = 0)
		: DrawableBase(layerDepth), DrawableString(text, fontFile, size, layerDepth), ScreenSpaceDrawable(alignH, alignV, layerDepth) {}

	ScreenString (std::string text, float size, AlignH alignH = LEFT, AlignV alignV = TOP, int layerDepth = 0)
